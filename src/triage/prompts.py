...
зараз нічого не треба.
  - поза скоупом — це взагалі не робота AI-юніту (закупівлі, HR-адміністрування, \
питання «куди подати заявку»), АБО це не запит (подяка, small talk, статус-апдейт).

Категорії перетинаються, тому за конфлікту застосовуй саме такий порядок \
пріоритету — перше правило, що спрацювало, виграє:
  1. Це не робота юніту або взагалі не запит → поза скоупом.
...
## needs_clarification
true, якщо запит надто розмитий, щоб брати його в роботу як є: незрозуміло, що \
саме треба зробити, для кого, або з якими даними. Одного лише слова «бот» чи \
«табличка» недостатньо, щоб почати роботу → true.
